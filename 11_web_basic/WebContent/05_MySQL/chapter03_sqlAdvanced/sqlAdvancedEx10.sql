...
/*
    ...
    - 서브쿼리를 잘못 사용하면 성능 저하를 초래할 수 있으므로 
      조인연산과 서브쿼리 방식을 상황에 맞게 사용하는 것이 중요하다.
...
SELECT CAR_CD
FROM   RENT
WHERE  RENT_PERIOD IN (SELECT MAX(RENT_PERIOD)
					   FROM   RENT);
...
WHERE
		R.RENT_PERIOD IN (SELECT MAX(RENT_PERIOD)
						  FROM   RENT);
...
# 예시 7) 한번도 렌트되지 않은 자동차 정보의 등록날짜를 '0000-00-00'으로 수정하기
UPDATE CAR_BAK
...
SELECT CAR_NM								AS CAR_NM,
	   PRICE								AS PRICE,
       (SELECT AVG(PRICE) FROM CAR)			AS AVG_PRICE,
       PRICE - (SELECT AVG(PRICE) FROM CAR) AS DEVIATION
FROM   CAR;
